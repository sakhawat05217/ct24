...
<body>
<?php
if(!isset($_SESSION['muser'])) echo "<script>window.location='../l/login.php?ref=m';</script>";
...
else if(isset($_POST['export_tables']))
{
 ...
 else
 {
	 ...
	 $command = system("mysqldump $DB_DATABASE $table_name -u $DB_USERNAME -p$DB_PASSWORD  -h $DB_HOST > $my_sql_file",$return_value);
	echo "<h3>Return Code: $return_value </h3>";
	if($return_value==0) echo "DB Export completed. ";
	else echo "Error Exporting the DB!";
...
else if(isset($_POST['import_db']))
{
 ...
 {
	$command = system("mysql -u $DB_USERNAME -p$DB_PASSWORD  -h $DB_HOST $DB_DATABASE < $my_sql_file",$return_value);
	echo "<h3>Return Code: $return_value </h3>";
...
foreach($data as $d)
{
	if($d!==null)
	{
		...
		$table_name_array[$d[0]]=count($table_data)-1;
	}
...
?>
<h2>Table Lists (<?= count($data)-1 ?>) <a style="text-decoration:none;" href="?&sort=<?= $sort ?>">Sort</a></h2>
