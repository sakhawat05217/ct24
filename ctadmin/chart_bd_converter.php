...
for($start_day=($day+$day_diff); $start_day>($day_diff-1); $start_day-=$day_diff)
{
    ...
    $sql= "select * from  ct_converter_bd where  created_at >= '$from_day' and created_at < '$day_limit' and converter_id = $converter_id";
    $result = mysqli_query($link,$sql) or die(mysqli_error($link));
    $data = mysqli_fetch_row($result);
...
$d_a[] = date('j M');
$day_like = $today;
$sql= "select * from  ct_converter_bd where  created_at like '%$day_like%' and converter_id = $converter_id";
$result = mysqli_query($link,$sql) or die(mysqli_error($link));
$data = mysqli_fetch_row($result);
if(null!==$data) $my_data= $data[2];
...
<input type="button" value="Refresh" class="btn btn-success" onclick="window.location.href='chart_bd_converter.php'" />
...
function show_chart()
{
	...
	window.location.href='chart_bd_converter.php?day='+day+'&converter_id=<?= $converter_id ?>';
