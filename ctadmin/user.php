...
if(isset($_POST['name']))
{
	...
	$sql = "insert into ct_users (name,email,password,status,role,created_at) value( '" . 
		$_POST['name'] . "', '".
		$_POST['email'] . "', '".
		$_POST['password'] . "', ". 
		$_POST['status'] . ", ".
...
{
	$rid = $_GET['del'];
	$sql= "delete from ct_users where id=$rid ";
	mysqli_query($link,$sql) or die(mysqli_error($link));
...
if(isset($_GET['s_str']))
{
    ...
    switch($sid)
	{
		...
		case 'bid': $sr_type = 'balance'; break;
	...
	$sql= "select * from ct_users where $sr_type like '%$s_string%'  order by $s_type $s_order";
}
else
{
	$sql= "select * from ct_users  order by $s_type $s_order";
}
...
<select name="sid" id="sid">
    ...
    <option value="bid" <?= ($sid=='bid')? ' selected="selected" ':'' ?>>Balance</option>
</select>
...
<a href="new_users.php" target="_blank" class="btn btn-info">New Users</a>    
...
echo '<div class="container"><div class="row"><div class="col-sm-1">#</div><div class="col-sm-2">'.get_column('name',$sid,$s_str,$row_limit).'</div><div class="col-sm-3">'.get_column('email',$sid,$s_str,$row_limit).'</div><div class="col-sm-1">'.get_column('status',$sid,$s_str,$row_limit).'</div><div class="col-sm-1">'.get_column('role',$sid,$s_str,$row_limit).'</div><div class="col-sm-2">'.get_column('balance',$sid,$s_str,$row_limit).'</div><div class="col-sm-2">Action</div></div><br>';
...
for($j=$low_limit; $j<=$high_limit; $j++)
{
    ...
    echo '<div class="col-sm-2"><input id="'.$new_data[0].'_balance" type="text" class="form-control" value="'.$new_data[9].'" /></div>';
...
function save_user(rid)
{
	...
	if(yn)
	{
        ...
        var balance = $('#'+rid+'_balance').val();
		window.location.href='user.php?p=<?= $current_page ?>&li=<?= $row_limit ?>&s_type=<?= $s_type ?>&s_order=<?= $s_order ?>&sid=<?= $sid ?>&s_str=<?= $s_str ?>&st='+status+'&r='+role+'&bal='+balance+'&sav='+rid;
