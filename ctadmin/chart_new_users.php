...
<div class="bg-secondary page-header">
    <div class="container">
        <h1 class="m-0">
       	 <div class="text-primary">New Users Chart Section</div>
        </h1>
    </div>
</div>
<br><br>	
...
else
{
    ...
    for($start_day=($day+$day_diff); $start_day>$day_diff; $start_day-=$day_diff)
    {
        ...
        $sql= "select * from ct_new_users where  created_at >= '$from_day' and created_at < '$day_limit' ";
        $result = mysqli_query($link,$sql) or die(mysqli_error($link));
        $data = mysqli_affected_rows($link);
        $u_a[] = $data;
...
<input type="button" value="Refresh" class="btn btn-success" onclick="window.location.href='chart_new_users.php'" />
...
function show_chart()
{
	...
	window.location.href='chart_new_users.php?day='+day;
