...
<div class="bg-secondary page-header">
    <div class="container">
        <h1 class="m-0">
       	 <div class="text-primary">View BD Currency Conversion</div>
        </h1>
    </div>
</div>
<br><br>	
...
<?php
$data_collection_sql= "select * from ct_currency_converter where target_country = 'BD'";
...
{
    $data_id = $data_collection_data[0];
    $target_country=$data_collection_data[1];
...
{
	$rid = $_GET['del'];
	$sql= "delete from ct_converter_bd where id=$rid ";
...
if((isset($_GET['s_str']))and(!empty($_GET['s_str'])))
{
    ...
    $sql= "select * from ct_converter_bd where $sr_type like '%$s_string%' $data_id_sql  order by $s_type $s_order,id asc limit 10000";

}
else
{
	$sql= "select * from ct_converter_bd where id > 0 $data_id_sql order by $s_type $s_order,id asc limit 10000";
...
<a href="chart_bd_converter.php?converter_id=<?= $d_id ?>" target="_blank" class="btn btn-warning">Chart</a>    
<input type="button" value="Refresh" class="btn btn-success" onclick="window.location.href='view_bd_converter.php'" />
...
<select onchange="window.location.href='view_bd_converter.php?p=<?= $current_page ?>&d_id=<?= $d_id ?>&li='+this.value">
...
?>
 ...
 <a class="<?= $active_class ?>" href="view_bd_converter.php?p=<?= $i ?>&li=<?= $row_limit ?>&sid=<?= $sid ?>&s_str=<?= $s_str ?>&s_type=<?= $s_type ?>&s_order=<?= $s_order ?>&d_id=<?= $d_id ?>"><?= $i ?></a>
...
function search_sid()
{
	...
	{
		window.location.href='view_bd_converter.php?p=1&li=<?= $row_limit ?>&s_type=<?= $s_type ?>&s_order=<?= $s_order ?>&sid='+sid+'&s_str='+s_str+'&d_id=<?= $d_id ?>';
	}
}
...
function delete_view_data_collection(rid)
{
	...
	if(yn)
	{
		...
		window.location.href='view_bd_converter.php?p=<?= $current_page ?>&li=<?= $row_limit ?>&s_type=<?= $s_type ?>&s_order=<?= $s_order ?>&sid=<?= $sid ?>&s_str=<?= $s_str ?>&del='+rid+'&d_id=<?= $d_id ?>';
	}
}
